...
public class GLBuffer implements AutoCloseable
{
	 ...
	 * {@code _platform_memmove} when {@code glBufferData}/{@code glBufferSubData}
	...
	protected void uploadBufferData(ByteBuffer bb, int bufferDataHint)
	{
		...
		if (shouldChunkUpload(bbSize))
		{
			...
			uploadInChunks(target, 0, bb, MAC_UPLOAD_CHUNK_BYTES);
		}
		else
		{
	 ...
	 */
	private static boolean shouldChunkUpload(int byteCount)
	{
		return byteCount > MAC_UPLOAD_CHUNK_THRESHOLD && EPlatform.get() == EPlatform.MACOS;
	}
	 ...
	 */
	private static void uploadInChunks(int target, int baseOffset, ByteBuffer bb, int chunkBytes)
	{
